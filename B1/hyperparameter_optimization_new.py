...
def evaluate_esn(Nres, p, alpha, rho, u_train, y_train, seed):
    ...
    if np.isnan(u_train).any() or np.isnan(y_train).any():
    ...
    # Ensure warmup is not longer than training
    warmup = 40000
    if warmup >= len(u_train):
        warmup = len(u_train) // 2
        print(f"Adjusted warmup to {warmup}")

    nrmse = esn.train(u_train, y_train, warmup=warmup)  # pass adjusted warmup
    return float(nrmse)
